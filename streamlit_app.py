...
def render_score_breakdown(score_breakdown: dict[str, str]) -> None:
    if not score_breakdown:
        st.caption("No score breakdown returned.")
        return
    rows = [
        {"Factor": key.replace("_", " ").title(), "Impact": value}
...
def render_analysis(analysis) -> None:
    ...
    with col_b:
        ...
        if analysis.recommended_consuming_weight:
            st.write(f"Recommended amount: **{analysis.recommended_consuming_weight}**")
    ...
    with tab_ingredients:
        ...
        render_list("Concerning Ingredients", analysis.concerning_ingredients)
    with tab_tips:
        render_list("Healthier Alternatives", analysis.healthier_alternatives)
        render_list("Balanced Diet Tips", analysis.balanced_diet_tips)


st.title("NutriScan AI")
st.caption("Upload ingredient-list and nutrition-chart images. Docling extracts structured text and tables before Groq analyzes the food.")
...
if ingredients_file or nutrition_file:
    ...
    with preview_left:
        if ingredients_file:
            st.image(Image.open(ingredients_file), caption="Ingredient list", use_container_width=True)
        else:
            st.info("Upload the ingredient-list image.")
    with preview_right:
        if nutrition_file:
            st.image(Image.open(nutrition_file), caption="Nutrition chart", use_container_width=True)
        else:
            st.info("Upload the nutrition-chart image.")

    can_scan = ingredients_file is not None and nutrition_file is not None

    if st.button("Scan labels", type="primary", use_container_width=True, disabled=not can_scan):
        ...
        with raw_left:
            ...
            st.text_area("Ingredient OCR output", raw_ingredients_text, height=260, label_visibility="collapsed")
        with raw_right:
            st.subheader("Nutrition OCR Text")
            st.text_area("Nutrition OCR output", raw_nutrition_text, height=260, label_visibility="collapsed")
